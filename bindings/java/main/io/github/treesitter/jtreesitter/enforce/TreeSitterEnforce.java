package io.github.treesitter.jtreesitter.enforce;

import java.lang.foreign.*;

public final class TreeSitterEnforce {
    ...
    private static final TreeSitterEnforce INSTANCE = new TreeSitterEnforce();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterEnforce() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_enforce");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-enforce");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-enforce");
