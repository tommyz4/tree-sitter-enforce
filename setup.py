...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_enforce", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_enforce": ["*.pyi", "py.typed"],
        "tree_sitter_enforce.queries": ["*.scm"],
    },
    ext_package="tree_sitter_enforce",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
